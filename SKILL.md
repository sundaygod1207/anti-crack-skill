...
| 环境变量 vs CLI | 上次把 `--license-token` 改为 `AI_MUSIC_TOKEN` 环境变量，Python 端还在读 `args.license_token` | 对比 JS 传递方式和 Python 接收方式 |
...
3. 环境变量 vs 命令行参数：对比 JS 传递方式和 Python 接收方式
   反面案例：JS 用 env.AI_MUSIC_TOKEN 传，Python 读 args.license_token → 永远为空
```
